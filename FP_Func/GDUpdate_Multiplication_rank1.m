function [ O, P ] = GDUpdate_Multiplication_rank1(O,P,dpsi,Omax,cen,Ps,alpha,beta,EPRY)
...
O(n1(1):n2(1),n1(2):n2(2)) = O(n1(1):n2(1),n1(2):n2(2))...
    + alpha*1/max(max(abs(P)))*abs(P).*conj(P).*dpsi./(abs(P).^2+5); %5 Siemens

if EPRY == 0
    P = P + 0;
elseif EPRY == 1
    P = P+beta*1/Omax*(abs(O1).*conj(O1)).*dpsi./(abs(O1).^2+1000).*Ps;
else
    disp('ERROR: opts.EPRY parameter not set')
end
